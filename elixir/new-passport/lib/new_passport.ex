defmodule NewPassport do
  def get_new_passport(now, birthday, form) do
    with {:ok, enter_time} <- enter_building(now),
         {:ok, manual} <- find_counter_information(now),
         counter <- manual.(birthday),
         {:ok, checksum} <- stamp_form(enter_time, counter, form),
         passport_number <- get_new_passport_number(enter_time, counter, checksum) do
      {:ok, passport_number}
    else
